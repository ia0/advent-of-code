...
fn reflect(map: &[Vec<u8>], mid: usize) -> bool {
    map[mid ..].iter().zip(map[.. mid].iter().rev()).map(|(x, y)| diff(x, y)).sum::<i64>() == 1
}

fn diff(xs: &[u8], ys: &[u8]) -> i64 {
    xs.iter().zip(ys.iter()).map(|(x, y)| (x != y) as i64).sum()
}
...
adventofcode::main!(solve("examples/13.txt") == "40995\n");
