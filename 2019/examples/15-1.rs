...
fn main() {
    ...
    todo.push_back(Frame { coord: Coord::default(), dist: 0, state: State::new(program) });
    ...
    while let Some(frame) = todo.pop_front() {
        for dir in 1 ..= 4 {
            ...
            assert!(visited.insert(coord));
            if push {
                todo.push_back(Frame { coord, dist, state });
            }
        }
    }
    unreachable!();
