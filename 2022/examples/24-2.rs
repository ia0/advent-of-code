...
fn solve(input: impl Read, mut output: impl Write) -> Result<()> {
    ...
    todo.push((0, Reverse(0), problem.start()));
    while let Some((phase, Reverse(time), pos)) = todo.pop() {
        if phase == 3 {
            writeln!(output, "{time}")?;
            break;
        }
        if !visited.insert((phase, time, pos)) {
            continue;
        }
        for &step in &STEPS {
            let pos = pos + step;
            if problem.is_step(time + 1, pos) {
                let phase = phase
                    + ((phase % 2 == 0 && pos == problem.end())
                        || phase == 1 && pos == problem.start()) as usize;
                todo.push((phase, Reverse(time + 1), pos));
            }
        }
    }
    Ok(())
...
adventofcode::main!(solve("examples/24.txt") == "809\n");
