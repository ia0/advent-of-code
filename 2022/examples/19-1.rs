...
fn solve(blueprint: &Blueprint) -> usize {
    ...
    todo.push(State { minutes: 24, ore: Resource { count: 0, robot: 1 }, ..State::default() });
    let mut best = 0;
    while let Some(cur) = todo.pop() {
        ...
        let cur = cur.geode.count + cur.minutes * cur.geode.robot;
...
fn entry(input: impl Read, mut output: impl Write) -> Result<()> {
    ...
    writeln!(
        output,
        "{}",
        blueprints.iter().enumerate().map(|(i, x)| (i + 1) * solve(x)).sum::<usize>()
    )?;
    Ok(())
}

adventofcode::main!(entry("examples/19.txt") == "1962\n");
