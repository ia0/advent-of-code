...
fn drop(map: &mut HashMap<Coord, Cell>, limit: i64, mut coord: Coord) -> bool {
    'drop: while coord.y < limit {
...
fn solve(input: impl Read, mut output: impl Write) -> Result<()> {
    ...
    let limit = map.keys().map(|c| c.y).max().unwrap();
    while drop(&mut map, limit, Coord { x: 500, y: 0 }) {}
    writeln!(output, "{}", map.values().filter(|x| matches!(x, Cell::Sand)).count())?;
    Ok(())
}

adventofcode::main!(solve("examples/14.txt") == "1061\n");
