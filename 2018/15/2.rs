...
impl Cell {
    ...
    fn is_dead(&mut self, elf_attack: u32) -> bool {
        match self {
            Unit { team, hit_points, .. } => {
                let attack = match team {
                    Gobelin => elf_attack,
                    Elf => 3,
                };
...
fn solve(
    mut cave: Vec<Vec<Cell>>, num_elf_orig: u32, mut num_gobelin: u32, elf_attack: u32,
) -> Option<u32> {
    let mut num_elf = num_elf_orig;
    let mut round = 0;
    loop {
        for x in 0 .. cave.len() {
            for y in 0 .. cave[x].len() {
                ...
                if let Some(enemy_pos) = compute_enemy(&cave, pos, team) {
                    if cave[enemy_pos.x][enemy_pos.y].is_dead(elf_attack) {
...
fn main() {
    ...
    for elf_attack in 4 ..= 200 {
        if let Some(outcome) = solve(cave.clone(), num_elf, num_gobelin, elf_attack) {
            println!("{}", outcome);
            break;
        }
    }
