...
fn solve(input: impl Read, mut output: impl Write) -> Result<()> {
    ...
    loop {
        ...
        let bot = slots.iter().find_map(|(slot, chips)| match slot {
    ...
    assert!((0 .. 3).all(|i| slots[&Slot::Output(i)].len() == 1));
    let result = (0 .. 3).map(|i| slots[&Slot::Output(i)][0]).product::<usize>();
    writeln!(output, "{result}")?;
    Ok(())
}

adventofcode::main!(solve("examples/10.txt") == "3965\n");
