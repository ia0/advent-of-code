...
fn score(ingredients: &[[i64; 5]], teaspoons: &[i64]) -> i64 {
    let mut r = [0; 5];
    ...
    r[.. 4].iter().map(|&x| std::cmp::max(0, x)).product()
}
...
fn solve(input: impl Read, mut output: impl Write) -> Result<()> {
    ...
    for line in BufReader::new(input).lines() {
        ...
        ingredients.push(<[i64; 5]>::try_from(words).unwrap());
    }
...
adventofcode::main!(solve("examples/15.txt") == "117936\n");
